...
function makeProvider(subscriptions) {
  ...
  const followSuggestionPath = (fromFile, suggestion) => {
    const resolveOptions = {
      ...
      moduleRoots: atom.config.get("js-hyperclick.moduleRoots"),
    }
...
module.exports = {
  config: {
    ...
    moduleRoots: {
      description: `
            Comma separated list of paths to aliased modules, relative to
            \`package.json\`. Useful if you are using webpack aliases. Use
            \`./src\`, for example, if you have an alias mapped to that directory.
            `.trim(),
      type: "array",
      default: [""],
      items: { type: "string" },
